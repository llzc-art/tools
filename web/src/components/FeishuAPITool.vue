<template>
  <div class="tool-panel">
    ...
    <div class="card">
      ...
      <div class="auth-grid">
        <div class="auth-field"><span class="auth-label">App ID</span><input v-model="appId" class="input-field" placeholder="cli_xxxxxxxx" /></div>
        <div class="auth-field"><span class="auth-label">App Secret</span><input v-model="appSecret" type="password" class="input-field" placeholder="App Secret" /></div>
      </div>
      <div class="token-row">
        <button @click="getToken" class="btn btn-outline" :disabled="tokenLoading">{{ tokenLoading ? '获取中...' : '获取 Tenant Token' }}</button>
      ...
      <div class="form-row">
        <div class="form-group" style="flex:1;min-width:90px"><label>方法</label>
          <select v-model="method" class="input-field"><option>GET</option><option>POST</option><option>PUT</option><option>DELETE</option></select>
        </div>
        <div class="form-group" style="flex:5"><label>请求路径</label><input v-model="apiPath" class="input-field" placeholder="/open-apis/..." /></div>
      </div>
      <div class="form-group"><label>Query 参数 <span class="hint">(每行 Key=Value)</span></label>
        <textarea v-model="queryParams" rows="3" class="input-field mono" placeholder="page_size=20&#10;department_id=0"></textarea>
      </div>
      <div class="form-group"><label>路径参数替换 <span class="hint">(每行 Key=Value，如 user_id=xxx)</span></label>
        <textarea v-model="pathParams" rows="2" class="input-field mono" placeholder="user_id=ou_xxxxx&#10;message_id=om_xxxxx"></textarea>
      </div>
      <div class="form-group"><label>请求体 <span class="hint">(JSON)</span></label>
        <textarea v-model="reqBody" rows="6" class="input-field mono" placeholder='{ "msg_type": "text", "content": { "text": "hello" } }'></textarea>
      </div>
      <div class="btn-group">
...
const appId = ref(''), appSecret = ref(''), accessToken = ref(''), tokenExpires = ref(0), tokenLoading = ref(false)
const apis = ref([]), selectedApiId = ref(''), method = ref('GET'), apiPath = ref('/open-apis/')
const queryParams = ref(''), pathParams = ref(''), reqBody = ref(''), loading = ref(false), resp = ref(null), error = ref('')
const BASE_URL = ref('https://open.feishu.cn')
...
async function loadAPIs() { const r = await apiGet('/api/integration/feishu/apis'); if (r.code===0) { const d=r.data; apis.value=d.apis||d; if(d.base_url)BASE_URL.value=d.base_url } }
...
async function getToken() {
  if(!appId.value||!appSecret.value){error.value='请填写 App ID 和 App Secret';return}
  tokenLoading.value=true;error.value=''
  try{
    const r=await apiPost('/api/integration/feishu/token',{app_id:appId.value,app_secret:appSecret.value})
    if(r.code===0&&r.data){const b=JSON.parse(r.data.body||'{}');if(b.tenant_access_token){accessToken.value=b.tenant_access_token;tokenExpires.value=b.expire||0}else{error.value=b.msg||'获取Token失败'}}
...
function buildURL() {
  ...
  // 替换路径参数 {xxx}
  if(pathParams.value.trim()){
    for(const line of pathParams.value.split('\n')){
      const i=line.indexOf('=');if(i>0){const k=line.substring(0,i).trim(),v=line.substring(i+1).trim();path=path.replace('{'+k+'}',encodeURIComponent(v))}
    }
  }
...
async function sendRequest() {
  ...
  try{
    const r=await apiPost('/api/integration/feishu/call',{method:method.value,url:buildURL(),headers:buildHeaders(),body:reqBody.value,timeout:30})
