<template>
  <div class="tool-panel">
    ...
    <div class="card">
      ...
      <div class="auth-grid">
        <div class="auth-field"><span class="auth-label">AppID</span><input v-model="appId" class="input-field" placeholder="wx1234567890abcdef" /></div>
        <div class="auth-field"><span class="auth-label">AppSecret</span><input v-model="appSecret" type="password" class="input-field" placeholder="App Secret" /></div>
      </div>
      <div class="token-row">
      ...
      <div class="form-group"><label>Query 参数 <span class="hint">(每行 Key=Value)</span></label>
        <textarea v-model="queryParams" rows="3" class="input-field mono" placeholder="openid=oXXXX&#10;lang=zh_CN"></textarea>
      </div>
      <div class="form-group"><label>请求体 <span class="hint">(JSON)</span></label>
        <textarea v-model="reqBody" rows="6" class="input-field mono" placeholder='{ "touser": "OPENID" }'></textarea>
      </div>
      <div class="btn-group">
...
const BASE_URL = ref('https://api.weixin.qq.com')
...
async function loadAPIs() { const r = await apiGet('/api/integration/wechat/apis'); if (r.code===0) { const d=r.data; apis.value=d.apis||d; if(d.base_url)BASE_URL.value=d.base_url } }
...
async function getToken() {
  if(!appId.value||!appSecret.value){error.value='请填写 AppID 和 AppSecret';return}
  tokenLoading.value=true;error.value=''
  try{
    const r=await apiPost('/api/integration/wechat/token',{app_id:appId.value,secret:appSecret.value})
...
async function sendRequest() {
  ...
  try{
    const r=await apiPost('/api/integration/wechat/call',{method:method.value,url:buildURL(),headers:{'Content-Type':'application/json'},body:reqBody.value,timeout:30})
